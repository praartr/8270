...
funcdef // Used in: decorated, compound_stmt
	: DEF NAME parameters COLON suite { TableManager::getInstance().pushScope();$$ = new FuncNode($2,$5);}
	;
...
expr_stmt // Used in: small_stmt
	...
	if($3->getNodetype() == 'V') {
	  a = new PlusExp( 
                         TableManager::getInstance().getEntry($1->getVariable()),
                         TableManager::getInstance().getEntry($3->getVariable())
                         );
	  SymbolTable s;
	  s.insert(
                                        $1->getVariable(),
...
print_stmt // Used in: small_stmt
	: PRINT opt_test {
	     ...
	     $$ = new PrintNode($2);
	     if($2->getNodetype() == 'V'){
		  if(TableManager::getInstance().checkName($2->getVariable()) ){
		      if((TableManager::getInstance().getEntry($2->getVariable()))->getNodetype() == 'F'){
			std::cout << std::fixed << std::setprecision(3) << "= " 
                                                << (TableManager::getInstance().getEntry($2->getVariable()))->getNumber() 
                                                << std::endl << ">>> "; 
		      }
                      else {
			std::cout << "= " 
